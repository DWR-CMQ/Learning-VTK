...
#include "Hash.h" // for token_NAMESPACE_* macros
#include "Exports.h" // for export macro
#include "Compiler.h" // for token_SPACE_BEFORE_SUFFIX
...
inline constexpr Hash operator"" _hash(
...
/// ```
#if token_SPACE_BEFORE_SUFFIX
inline constexpr Token operator"" _token(
  const char* data, std::size_t size)
#else
inline constexpr Token operator""_token(
  const char* data, std::size_t size)
...
/// containers.
template <>
struct hash<token_NAMESPACE::Token>
{
