...
#include <vtkMath.h>
#include <glad/glad.h>
OpacityTable::OpacityTable(int dataWidth, int dataHeight, int dataDepth)
{
	m_iWidth = dataWidth;
...
int OpacityTable::GetMaximumSupportedTextureWidth(int idealWidth)
{
...
void OpacityTable::ReleaseGraphicsResources()
{
	if (this->m_pTextureObject != NULL)
	{
...
void OpacityTable::Update(std::shared_ptr<OpacityTransferfunction> func, double scalarRange[2], int blendMode, double sampleDistance, double unitDistance, int filterValue)
{
	if (func == nullptr)
	{
		return;
...
bool OpacityTable::NeedUpdate(std::shared_ptr<OpacityTransferfunction> func, double scalarRange[2], int blendMode, double sampleDistance)
{
	if (func == nullptr)
	{
		return false;
...
void OpacityTable::InternalUpdate(std::shared_ptr<OpacityTransferfunction> func, int blendMode, double sampleDistance, double unitDistance, int filterValue)
{
	if (func == nullptr)
	{
		return;
	...
	if (this->LastBlendMode == TextureObject::COMPOSITE_BLEND)
	{
	...
	else if (blendMode == TextureObject::ADDITIVE_BLEND)
	{
		float* ptr = this->Table;
		double factor = sampleDistance / unitDistance;
		int i = 0;
		if (*ptr > 0.0001f)
		{
			*ptr = static_cast<float>(pow(1.0 - static_cast<double>(*ptr), factor));
		}
		++ptr;
		++i;
	}
...
void OpacityTable::ComputeIdealTextureSize(std::shared_ptr<OpacityTransferfunction> func, int& width, int& height)
{
	if (func != NULL)
	{
...
void OpacityTable::AllocateTable()
{
	delete[] this->Table;
